...
# treating empty data
df["Death Rate"] = df["Death Rate"].apply(lambda x: 0 if isinstance(x, str) and x == "nan" else x)
df["Daily Cases"].fillna(0, inplace=True)
df["Daily Deaths"].fillna(0, inplace=True)
df["New Recoveries"].fillna(0, inplace=True)

df["Vaccination"] = df["Date"].apply(lambda x : 0 if x < pd.to_datetime("2021-01-17") else 1)
